...
public class ValidationAttributeDeclarativeExtractorTests
{
    [Fact]
    public void New_ReturnOK()
    {
        Assert.True(
            typeof(IHttpDeclarativeExtractor).IsAssignableFrom(typeof(ValidationAttributeDeclarativeExtractor)));

        var extractor = new ValidationAttributeDeclarativeExtractor();
    ...
    public void Extract_ReturnOK()
    {
        ...
        Assert.Throws<ValidationException>(() =>
            new ValidationAttributeDeclarativeExtractor().Extract(httpRequestBuilder1, context1));

        var context2 = new HttpDeclarativeExtractorContext(method1, ["furion", null]);
        Assert.Throws<ValidationException>(() =>
            new ValidationAttributeDeclarativeExtractor().Extract(httpRequestBuilder1, context2));

        var context3 =
            new HttpDeclarativeExtractorContext(method1, ["furion", new ValidationModel { Name = "furion" }]);
        new ValidationAttributeDeclarativeExtractor().Extract(httpRequestBuilder1, context3);
    }
    ...
    public void ValidateParameter_Invalid_Parameter()
    {
        Assert.Throws<ArgumentNullException>(() =>
            ValidationAttributeDeclarativeExtractor.ValidateParameter(null!, null));
        ...
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters[0], null);
        });
        Assert.Equal("The str field is required.", exception.Message);

        var exception2 = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters[1], null);
        });
    ...
    public void ValidateParameter_ReturnOK()
    {
        ...
        var exception = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters[1], new ValidationModel());
        });
        Assert.Equal("The Name field is required.", exception.Message);

        var exception2 = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters[1],
        ...
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[0], null);
        });
        Assert.Equal("The str field is required.", exception3.Message);

        var exception31 = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[0], "a");
        });
        ...
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[0], "furion");
        });
        Assert.Equal("The field str must be a string or array type with a maximum length of '5'.", exception32.Message);

        ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[0], "fur");

        var exception4 = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[1], 12);
        });
        Assert.Equal("The field age must be between 0 and 10.", exception4.Message);

        var exception41 = Assert.Throws<ValidationException>(() =>
        {
            ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[1], -1);
        });
        Assert.Equal("The field age must be between 0 and 10.", exception41.Message);

        ValidationAttributeDeclarativeExtractor.ValidateParameter(parameters2[1], 5);
    }
}
